// src/components/tenants/TenantCardGrid.tsx
"use client";
...
interface TenantCardGridProps<T> {
  data: T[];
...
export default function TenantCardGrid<T>({
  data,
  renderCardHeader,
  renderCardBody,
  renderCardFooter,
    ...
    return (
      <div className="space-y-3 p-4">
        {[...Array(count)].map((_, i) => (
          <div 
            key={i} 
            className={`p-4 bg-[var(--color-surface-hover)]/40 border border-[var(--color-surface-border)] animate-pulse ${
              compact ? 'p-3' : 'p-4'
            }`}
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0 flex-1">
                <div className="w-10 h-10 bg-[var(--color-surface)] border border-[var(--color-surface-border)] flex-shrink-0" />
                <div className="min-w-0 flex-1 space-y-2">
                  <div className="h-4 bg-[var(--color-surface-hover)] w-3/4" />
                  <div className="h-3 bg-[var(--color-surface-hover)] w-1/2" />
                </div>
              </div>
              <div className="w-8 h-8 bg-[var(--color-surface-hover)] flex-shrink-0" />
            </div>
          </div>
        ))}
      </div>
  ...
  const renderEmptyState = () => (
    <div className="flex items-center justify-center h-full min-h-[200px] p-12 text-center">
      <div>
        <div className="w-16 h-16 bg-[var(--color-surface-hover)] border border-[var(--color-surface-border)] flex items-center justify-center mx-auto mb-4">
        ...
        if (isMobile) {
          return createPortal(
            <>
              ...
              <div
                className="fixed left-0 right-0 z-[9999] bg-[var(--color-surface)] border-t border-[var(--color-surface-border)] 
              ...
              >
                <div className="flex justify-center pt-3 pb-2">
                  <div className="w-12 h-1.5 bg-[var(--color-surface-border)] opacity-60" />
                </div>
                ...
                <div className="flex items-center justify-between px-5 py-3 border-b border-[var(--color-surface-border)]">
                  ...
                  <button
                    ...
                    className="p-2 bg-[var(--color-surface-hover)] hover:bg-[var(--color-surface-border)] transition-all active:scale-95"
                    aria-label="Close"
                  >
          ...
          return createPortal(
            <>
              ...
              <div
                ...
                className="fixed z-[9999] min-w-[180px] max-w-[280px] bg-[var(--color-surface)] border border-[var(--color-surface-border)] shadow-[var(--shadow-dropdown)] overflow-hidden animate-in fade-in zoom-in-95 duration-100"
                style={{ 
                  top: dropdownPos.top, 
